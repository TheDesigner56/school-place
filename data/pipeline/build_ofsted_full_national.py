#!/usr/bin/env python3
"""National Ofsted layer: per-school latest + previous graded inspection from the
outcomes file, merged with new-framework report cards from ofsted_latest.json.

Same shape as build_ofsted_full.py (pilot), but for ALL national schools.
"""
...
LATEST_JSON = os.path.join(BASE, "web/public/data/ofsted_latest.json")

conn = sqlite3.connect(os.path.join(BASE, "data/processed/national.db"))
urns = {str(u) for (u,) in conn.execute("SELECT urn FROM schools")}
...
print(f"schools: {len(out)} | with previous inspection: {with_prev} | with report URL: {with_url} | new report cards: {with_cards}")
