...
export default async function SchoolPage({ params }: { params: { slug: string } }) {
  ...
  if (schoolPerf?.ks2?.expected_pct != null) {
    stats.push({ label: "KS2 expected std", value: `${schoolPerf.ks2.expected_pct}%`, provenance: schoolPerf.source_label });
  }
  ...
  if (stats.length < 4 && chars?.eal_pct != null) {
    stats.push({ label: "English as addl. lang", value: `${chars.eal_pct}%`, provenance: chars.source_label });
  }
    ...
    <div>
      ...
      <div className="container max-w-3xl py-6 sm:py-10">
        ...
        <section className="border-b border-border/60 pb-8">
          <p className="eyebrow">
            {school.phase} · {districtLabel(district)} · {school.la}
          </p>
          ...
          {derived && (
            <p className="mt-5 flex items-start gap-2.5 rounded-2xl border border-primary/20 bg-primary/[0.06] px-4 py-3 text-[13px] leading-relaxed text-muted-foreground">
        ...
        {/* ── Stat strip — Flighty-grade numerals, provenance on every figure ── */}
        {stats.length > 0 && (
          <section className="mt-8 grid grid-cols-2 gap-2.5 sm:grid-cols-4 sm:gap-3">
            {stats.slice(0, 4).map((s, i) => (
              ...
              >
                <p className="eyebrow text-[9.5px]">{s.label}</p>
                <p className="mt-2 font-serif text-[1.9rem] font-semibold leading-none tabular-nums tracking-tight">{s.value}</p>
                {s.provenance && <ProvenanceChip className="mt-3">{s.provenance}</ProvenanceChip>}
              </div>
            ))}
          </section>
        )}
        ...
        <div className="mt-8 space-y-6">
          ...
          <Card>
            <CardHeader>
              <CardTitle>Ofsted</CardTitle>
              <CardDescription>Inspection history &amp; judgements</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {derived && ofsted?.latest && (
                <div className="rounded-xl border border-border/70 bg-muted/50 px-4 py-3 text-sm">
              ...
              <div className="space-y-4">
                {ofsted?.latest?.date && (
                  ...
                  >
                    {ofsted.latest.sub && (
                      <dl className="mt-2.5 grid grid-cols-2 gap-x-3 gap-y-1.5 text-xs sm:grid-cols-3">
                ...
                <div className="rounded-xl border border-border/70 bg-muted/30 p-3.5">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {Object.entries(ofsted.report_card_2026.sub_judgements).map(([k, v]) => (
                      <span
                        key={k}
                        className="inline-flex items-center gap-1.5 rounded-full border border-border/70 bg-card px-2.5 py-1 text-xs font-medium"
                      >
                        <span
              ...
              {ofsted?.report_url && (
                <a href={ofsted.report_url} target="_blank" rel="noopener noreferrer"
                  ...
                  <BookOpen className="h-3.5 w-3.5" /> Full report on Ofsted.gov.uk ↗
                </a>
              )}
              <ProvenanceLine>
                ...
                <>
                  {schoolPerf.ks2 && (
                    <dl className="grid grid-cols-2 gap-x-4 gap-y-4 text-sm sm:grid-cols-3">
                      <Metric label="Expected standard (RWM)" value={schoolPerf.ks2.expected_pct != null ? `${schoolPerf.ks2.expected_pct}%` : "—"} />
                      <Metric label="Higher standard" value={schoolPerf.ks2.higher_pct != null ? `${schoolPerf.ks2.higher_pct}%` : "—"} />
                      {schoolPerf.ks2.reading_avg != null && (
                        <Metric label="Avg scaled score (reading)" value={String(schoolPerf.ks2.reading_avg)} />
                      )}
                    </dl>
                  )}
                  {schoolPerf.gcse && (
                    <dl className="grid grid-cols-2 gap-x-4 gap-y-4 text-sm sm:grid-cols-4">
                      <Metric label="Attainment 8" value={schoolPerf.gcse.attainment8 != null ? String(schoolPerf.gcse.attainment8) : "—"} />
                      <Metric label="Progress 8" value={schoolPerf.gcse.progress8 != null ? schoolPerf.gcse.progress8.toFixed(2) : "—"} />
                      <Metric label="EBacc APS" value={schoolPerf.gcse.ebacc_aps != null ? String(schoolPerf.gcse.ebacc_aps) : "—"} />
                      <Metric label="Grade 5+ Eng & Maths" value={schoolPerf.gcse.eng_maths_5plus_pct != null ? `${schoolPerf.gcse.eng_maths_5plus_pct}%` : "—"} />
                    </dl>
                  )}
                  {trendYears.length > 1 && (
              ...
              {admYears.length > 0 ? (
                <>
                  {distMin != null && distMax != null && (
                    <div>
                      <p className="font-serif text-[1.7rem] font-semibold leading-none tabular-nums tracking-tight">
                        {fmtDist(distMin)} <span className="text-muted-foreground">–</span> {fmtDist(distMax)}
                      </p>
                  ...
                  <dl className="space-y-1 text-sm">
                    {admYears.map((y) => (
                      <div key={y.year} className="flex items-center justify-between gap-3 border-b border-border/40 py-2 last:border-0">
                        <dt className="font-mono text-xs text-muted-foreground tabular-nums">{y.year}</dt>
                        ...
                        {y.pan != null && <dd className="font-mono text-[10px] text-muted-foreground tabular-nums">PAN {y.pan}</dd>}
                      </div>
                    ))}
                  </dl>
                  <ProvenanceLine>
          ...
          <Card>
            <CardHeader>
              <CardTitle>Pupil profile</CardTitle>
            ...
            <CardContent className="space-y-4">
              {chars && (chars.fsm_pct != null || chars.ethnicity) ? (
                <>
                  <dl className="grid grid-cols-2 gap-x-4 gap-y-4 text-sm sm:grid-cols-4">
                    <Metric label="Free school meals" value={chars.fsm_pct != null ? `${chars.fsm_pct}%` : "—"} />
                    <Metric label="English as addl. lang" value={chars.eal_pct != null ? `${chars.eal_pct}%` : "—"} />
                    <Metric label="Avg class size" value={chars.class_size_avg != null ? String(chars.class_size_avg) : "—"} />
                    <Metric label="On roll" value={chars.pupils_on_roll != null ? String(chars.pupils_on_roll) : "—"} />
                  </dl>
                  {chars.ethnicity && (() => {
                    ...
                    return top.length > 0 ? (
                      <div>
                        <p className="mb-2 text-xs text-muted-foreground">Ethnicity (largest groups)</p>
          ...
          <Card>
            ...
            <CardContent>
              <dl className="grid grid-cols-2 gap-x-4 gap-y-3 text-sm sm:grid-cols-4">
                <div><dt className="text-xs text-muted-foreground">Ward</dt><dd className="mt-1 font-medium">{school.ward ?? "—"}</dd></div>
                <div><dt className="text-xs text-muted-foreground">District</dt><dd className="mt-1 font-medium">{school.district ?? "—"}</dd></div>
                <div><dt className="text-xs text-muted-foreground">Postcode area</dt><dd className="mt-1 font-medium">{school.postcode}</dd></div>
                <div><dt className="text-xs text-muted-foreground">Coordinates</dt><dd className="mt-1 font-mono text-xs tabular-nums">{school.lat != null ? school.lat.toFixed(4) : "—"}, {school.lng != null ? school.lng.toFixed(4) : "—"}</dd></div>
              </dl>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              {flood ? (
                <>
                  {flood.flood_zone3 ? (
                    <RiskPanel tone="rose" title="In Flood Zone 3 — high risk"
                      body="Land assessed as having a 1-in-100 (river) or 1-in-200 (sea) annual chance of flooding." />
                  ) : flood.flood_zone2 ? (
                    <RiskPanel tone="amber" title="In Flood Zone 2 — medium risk"
                      body="Land assessed as having between a 1-in-100 and 1-in-1,000 annual chance of flooding." />
                  ) : (
                    <RiskPanel tone="emerald" title="Outside Flood Zones 2 & 3"
                      body="Not in the Environment Agency's assessed flood zones. Local surface-water risk may still apply." />
                  )}
                  <ProvenanceLine>
          ...
          <Card>
            <CardHeader>
              <CardTitle>Neighbourhood safety</CardTitle>
              <CardDescription>
              ...
              {lsoaCrime ? (
                <div className="space-y-4">
                  <div className="flex items-baseline gap-2.5">
                    <span className="font-serif text-[1.9rem] font-semibold leading-none tabular-nums tracking-tight">{lsoaCrime.total}</span>
                    <span className="text-sm text-muted-foreground">crimes recorded in one month</span>
                  </div>
                  <dl className="space-y-1 text-sm">
                    {topCategories.map(([cat, n]) => (
                      <div key={cat} className="flex items-center justify-between gap-3 border-b border-border/40 py-2 last:border-0">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              {districtPrices && districtPrices.median != null ? (
                <>
                  <div className="flex items-baseline gap-2.5">
                    <span className="font-serif text-[1.9rem] font-semibold leading-none tabular-nums tracking-tight">£{districtPrices.median.toLocaleString()}</span>
          ...
          <Card>
            <CardHeader>
              <CardTitle>Neighbourhood</CardTitle>
            ...
            <CardContent>
              {nearby.length > 0 ? (
                <ul className="divide-y divide-border/50">
                  {nearby.map((s) => (
                    <li key={s.urn}>
                      <Link href={`/school/${s.slug}`} className="-mx-2 flex items-center justify-between gap-3 rounded-xl px-2 py-3 text-sm transition-colors hover:bg-accent/60">
              ...
              <Link href={`/area/${district}`} className="mt-4 inline-flex items-center gap-1.5 text-[13px] font-medium text-primary underline-offset-4 hover:underline">
...
function SummaryChip({ icon: Icon, value }: { icon: React.ElementType; value: string }) {
  return (
    <span className="inline-flex items-center gap-1.5 rounded-full border border-border/70 bg-card px-3 py-1.5 text-xs font-medium shadow-card">
...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <dd className="mt-1 font-serif text-xl font-semibold tabular-nums tracking-tight">{value}</dd>
    </div>
  );
}

function RiskPanel({ tone, title, body }: { tone: "rose" | "amber" | "emerald"; title: string; body: string }) {
  const tones = {
  ...
  return (
    <div className={`flex items-center gap-3 rounded-xl border p-3.5 ${tones[tone]}`}>
      <span className="text-xl">{tone === "emerald" ? "✓" : "⚠"}</span>
      <div>
...
function TimelineItem({
  ...
  return (
    <div className="relative flex gap-3.5 pl-5">
      <span className="absolute left-0 top-1.5 h-2 w-2 rounded-full bg-primary/50 ring-4 ring-primary/10" />
      <span className="absolute bottom-0 left-[3.5px] top-5 w-px bg-border" />
      <div className="flex-1 pb-2">
        <div className="flex flex-wrap items-center gap-2">
          <span className="font-mono text-xs tabular-nums text-muted-foreground">{date}</span>
...
function CompactEmpty({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-center justify-center rounded-2xl border border-dashed border-border bg-muted/30 px-4 py-7 text-center text-sm leading-relaxed text-muted-foreground">
      {children}
    </div>
  );
}
